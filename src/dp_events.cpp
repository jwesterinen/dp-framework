...
#include <arpa/inet.h>
#include <sys/time.h>
#include "../include/dp_events.h"

namespace DP
{

int EventContext::cmdfd = -1;
...
Callback::Callback(unsigned _period) : period(_period)
{
	...
	// convert to timeout
	to = EventContext::Tv2us(&tv) + (period * 1000);
	period = period * 1000;
}
...
EventContext::EventContext() : maxFd(0)
{
	// init the global command FD
    EventContext::cmdfd = OpenConnection();
}
 ...
 ***************************************************************************/
unsigned long long EventContext::Tv2us(struct timeval *ptv)
{
    return ((((unsigned long long)ptv->tv_sec) * 1000000) + ptv->tv_usec);
}

int EventContext::OpenConnection()
{
...
void EventContext::Register(SelectableSensor* sensor)
{
...
void EventContext::Register(Callback* routine)
{
 ...
 * Effects:      none
 ***************************************************************************/
timeval* EventContext::ExecRoutines()
{
...
void EventContext::MainEventLoop()
{
    fd_set readset;
